...
const playgroundController = async (req, res) => {
    try {
        const apiKey = req.params.apiKey;
        if (!apiKey) {
            return res.status(403).json({success:false,msg:'Api Key is not provided!'})
        ...
        try {
            playgroundPrompts = [...playgroundPrompts, { role: "user", content: JSON.stringify(req.body) }]
        } catch (error) {
            return res.status(400).json({success:false,msg:error.message})
        }
        const openai = new OpenAI({
...
module.exports = {dataController, playgroundController}